  ...
  'TEI': {
    ...
    'metadata': {
      'articleTitle': function($) {
        return $('TEI > teiHeader > fileDesc > titleStmt > title[type="main"]').text();
      },
      'publisher': function($) {
      ...
      'author_organizations': function($) {
        return $('TEI > teiHeader > fileDesc > sourceDesc > biblStruct > analytic > author > affiliation > orgName')
          .map(function(i, el) {
            return $(el).text();
          })
          .get();
      ...
      'pmcid': function($) {
        return $('TEI > teiHeader > fileDesc > sourceDesc > biblStruct > idno[type="PMCID"]').text();
      },
      'publication_date': function($) {
        let date1 = $(
            'TEI > teiHeader > fileDesc > sourceDesc > biblStruct > monogr > imprint > date[type="published"]'
          ).text(),
          date2 = $('TEI > teiHeader > fileDesc > publicationStmt > date[type="published"]').text();
        return date1.length > 0 ? date1 : date2;
      }
    }
  }
};
